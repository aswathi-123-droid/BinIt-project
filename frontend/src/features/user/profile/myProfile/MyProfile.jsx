      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-3xl min-w-4xl bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-8 py-6 border-t border-gray-50">
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-3">
                ...
                />
                <button onClick={handleEmailTrigger} type="button" className={`px-6 py-3 border border-gray-200 rounded-xl text-sm font-bold flex items-center gap-2 ${emailTrigger?" bg-emerald-500  hover:bg-emerald-600 text-white":" hover:bg-gray-50"}`}>
                  {emailTrigger?"":<Edit2 size={16} />}{emailTrigger?"Cancel":" Change Email"}
                </button>
              </div>
            </div>
          ...
          <div className="flex justify-end items-center gap-4 mt-10">
            <button type="button" onClick={() => reset()} className="text-sm font-bold text-gray-500">Cancel</button>
            <button 
              type="submit" 
              disabled={!isDirty || loading}
              className={`px-8 py-3 text-white text-sm font-bold rounded-xl shadow-md ${
                isDirty && !loading ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-gray-300'
              }`}
            >
