...
const MyWallet = () => {
  ...
  const { data: walletData, isLoading, isError, refetch } = useQuery({
    queryKey: ['myWallet'],
    queryFn: async () => {
  ...
  const totalPages = Math.max(1, Math.ceil(allTransactions.length / itemsPerPage));
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center py-32 space-y-4">
        ...
        <p className="text-gray-500 font-medium animate-pulse">Loading wallet securely...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <h3 className="text-xl font-bold text-gray-900">Failed to load wallet</h3>
        <p className="text-gray-500 mt-2 mb-6">We couldn't retrieve your wallet details at the moment.</p>
        <button 
          onClick={refetch}
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 lg:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Manage your earnings, refunds, and pay quickly on your next request.</p>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm pt-6 pl-6 pr-6">
        ...
        {allTransactions.length === 0 ? (
          <div className="py-12 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="text-sm mt-1">When you sell scrap or use wallet for orders, they will appear here.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4 md:gap-5">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${isCredit ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                    ...
                    <div>
                      <p className="font-bold text-gray-900 text-sm md:text-base capitalize">
                        {tx.description || tx.transactionReason?.replace(/_/g, ' ') || 'Wallet Transaction'}
                      </p>
                  ...
                  <div className={`text-right font-black md:text-lg ${isCredit ? 'text-emerald-600' : 'text-gray-900'}`}>
