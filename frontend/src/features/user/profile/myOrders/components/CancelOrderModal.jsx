    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-125 p-8 relative shadow-xl transform transition-all scale-100">
        ...
        <p className="text-sm text-gray-500 mb-6 leading-relaxed">
          Please tell us why you are canceling your order{" "}
        ...
        <div className="space-y-4 mb-6">
          {reasons.map((reason) => (
            ...
            >
              ...
              <span
                className={`text-sm ${selectedReason === reason ? "text-gray-900 font-medium" : "text-gray-600"}`}
              >
                {reason}
              </span>
            </div>
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out ${selectedReason === "Other (please specify)" ? "max-h-40 opacity-100 mb-6" : "max-h-0 opacity-0"}`}
        >
          <textarea
