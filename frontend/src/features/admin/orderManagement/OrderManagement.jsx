...
const AdminOrderManagement = ({ mode = 'order' }) => {
    ...
    useEffect(() => {
        let id = setTimeout(() => { setSearchInput(search); }, 500);
        return () => clearTimeout(id);
    }, [search]);

    // Fetch data using React Query
    const { data, isLoading, isError } = useQuery({
        queryKey: [isPickupMode ? "admin_pickups" : "admin_orders", searchInput, page, statusFilter, sortBy],
        queryFn: async () => {
            const endpoint = isPickupMode ? "/admin/order/pickups" : "/admin/order/orders";
    ...
    return (
        <div className="p-4 md:p-8 bg-gray-50 min-h-screen font-sans">
            ...
            {/* Header Title (Optional, matches screenshot) */}
            <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {stats.map((stat, index) => (
                    <div key={index} className="bg-white p-6 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.08)] border border-gray-100 flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-gray-500 mb-1">{stat.label}</p>
                            <h3 className={`text-2xl font-extrabold ${stat.valueColor || 'text-slate-800'}`}>
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.08)] border border-gray-100 overflow-hidden">
                ...
                <div className="p-5 border-b border-gray-100">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        {/* Filters & Actions Group (Right aligned) */}
                        <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto">
                            ...
                            <div className="relative border border-gray-200 rounded-xl px-4 py-2.5 flex items-center gap-2 bg-white min-w-40 cursor-pointer hover:bg-gray-50 transition-colors">
                                <span className="text-sm font-medium text-gray-500">
                                    Filter by <span className="text-slate-700 font-semibold capitalize">
                                        {!statusFilter ? "Status" : statusFilter}
                                    </span>
                                </span>
                                <ChevronDown size={16} className="text-gray-400 ml-auto" />
                                <select onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="absolute inset-0 opacity-0 cursor-pointer">
                            ...
                            <div className="relative border border-gray-200 rounded-xl px-4 py-2.5 flex items-center gap-2 bg-white min-w-40 cursor-pointer hover:bg-gray-50 transition-colors">
                                <span className="text-sm font-semibold text-slate-700 capitalize">
                                    {sortBy === 'newest' ? 'Newest First' : sortBy === 'oldest' ? 'Oldest First' : 'Amount'}
                                </span>
                                <ChevronDown size={16} className="text-gray-400 ml-auto" />
                                <select onChange={(e) => { setSortBy(e.target.value); setPage(1); }} className="absolute inset-0 opacity-0 cursor-pointer">
                                ...
                                {data?.items?.length > 0 ? (
                                    data.items.map((order, index) => (
                                        <tr key={order._id} className="hover:bg-gray-50/50 transition-colors group">
                                            ...
                                            <td className="px-6 py-5 text-center">
                                                <button
                                                onClick={()=> navigate(`/admin/${isPickupMode?"pickups":"orders"}/${order._id}`)} 
