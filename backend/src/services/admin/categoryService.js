...
export const createCategory = async (categoryData) => {
  ...
  logger.info(
    `Category created successfully: ${newCategory.name} [ID: ${newCategory._id}]`,
  );
...
export const getAllCategories = async (queryParams) => {
  const { page, limit, status, type, search, sortBy = "newest" } = queryParams;
  ...
  const [categories, totalCount, recyclableCount, junkCount, storeCount] =
    await Promise.all([
      categoriesPromise,
...
export const updateCategoryOffer = async (categoryId, offerData) => {
  ...
  if (!category) {
    logger.warn(
      `Service: Update offer failed. Category not found [ID: ${categoryId}]`,
    );
    throw new AppError(
      STATUS_CODES.NOT_FOUND,
      "CATEGORY_NOT_FOUND",
  ...
  logger.info(
    `Service: Offer updated successfully for category '${category.name}'`,
  );
