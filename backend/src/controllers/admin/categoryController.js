...
import { createCategory, createCategoryOffer, getAllCategories, updateCategory, updateCategoryOffer, updateCategoryStatus } from "../../services/admin/categoryService.js";
...
export const updateCategoryController = async (req, res) => {
  ...
  if (file) {
    logger.info(`Controller: Processing new image upload for category [${categoryId}]`);
    imageUrl = await uploadToCloudinary(file.path);
  } else if (req.body.image) {
    imageUrl = req.body.image;
  } 
...
export const updateCategoryStatusController = async(req,res) => {
  ...
  logger.info(`Controller: Toggling status for category [${categoryId}] by Admin [${req.user?._id}]`);
  const updatedCategory = await updateCategoryStatus(categoryId);
  logger.info(`Controller: Category '${updatedCategory.name}' is now [${updatedCategory.isActive ? 'ACTIVE' : 'INACTIVE'}]`);
  sendResponse(res,
    {
...
export const updateCategoryOfferController = async(req,res) => {
    ...
    const offerData = req.body;
     ...
     logger.info(`Controller: Request to update offer for category [${categoryId}] by Admin [${req.user?._id}]`);
